 ...
 * Supports static images (PNG, JPG, WebP) and animated images (GIF, APNG) for premium users
 * @param {File} file - The avatar image file
 * @returns {Promise<string>} - The public URL of the uploaded avatar
 */
export async function uploadUserAvatar(file) {
  // Validate file size (max 10MB for animated, 5MB for static)
  const isAnimated = file.type === 'image/gif' || file.name.toLowerCase().endsWith('.apng');
  const maxSize = isAnimated ? 10 * 1024 * 1024 : 5 * 1024 * 1024;
  
  if (file.size > maxSize) {
    throw new Error(`Avatar must be less than ${isAnimated ? '10MB' : '5MB'}`);
  }

  // Validate file type - allow all common image formats including animated
  const validTypes = ['image/png', 'image/jpeg', 'image/jpg', 'image/gif', 'image/webp', 'image/apng'];
...
export async function uploadBanner(file) {
  // Validate file size (max 15MB for animated, 10MB for static)
  const isAnimated = file.type === 'image/gif' || file.name.toLowerCase().endsWith('.apng');
  const maxSize = isAnimated ? 15 * 1024 * 1024 : 10 * 1024 * 1024;
  
  if (file.size > maxSize) {
    throw new Error(`Banner must be less than ${isAnimated ? '15MB' : '10MB'}`);
  }
