...
export const api = {
  ...
  async sendMessage(channel, text, author, attachments = null) {
    const token = localStorage.getItem('token');
  ...
  async changeDiscriminator(userId, discriminator) {
    // This endpoint no longer exists - usernames are now unique
    throw new Error('Discriminators have been removed. Usernames are now unique.');
  },
