...
function MessageInput({ onSendMessage, onTyping, channel, members = [], placeholder, replyTo, onCancelReply }) {
    ...
    <div className="message-input-container">
      ...
      {replyTo && (
        <div className="reply-banner">
          <ArrowBendUpLeft size={14} className="reply-banner-icon" />
          <span className="reply-banner-label">Replying to <strong>{replyTo.author}</strong></span>
          <span className="reply-banner-preview">{replyTo.text?.slice(0, 60)}{replyTo.text?.length > 60 ? '…' : ''}</span>
          <button className="reply-banner-close" onClick={onCancelReply}><X size={14} /></button>
        </div>
      )}
